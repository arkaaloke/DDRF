...
def DRF_twice( jobfile, stopTime=30000, logfile="delaysDDRF.csv"):
	global INF
	elephantNumTasks = 10000
...
def start_exp(exp_type, machine_types, fraction_mice_servers, elephant_num_tasks_threshold):
    ...
    if exp_type == "drf":
        sim.DRF(jobfile, sys.maxint, sys.maxint )
    elif exp_type == "ddrf":
        sim.DDRF( jobdile, sys.maxint, sys.maxint )
...
if __name__== "__main__":
    ...
    start_exp(sys.argv[1], sys.argv[2], float(sys.argv[3]), int(sys.argv[4]))
